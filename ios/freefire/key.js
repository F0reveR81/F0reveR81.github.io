        ...
        function login(km, mm = "", funName = "wanggou") {
            return new Promise(async (resolve, reject) => {
                try {
                    ...
                    const url = `${appConfig.uri}/${
              Math.floor(new Date().getTime() / 1000) - 3
            }/${appConfig.appid}/${km}?${params}`;
                    const loginData = await getDataByJsonP("login", url, "login_data");
                    // 解密返回数据
                    ...
                    if (
                        !wigdata.check ||
                        wigdata.check != md5(imei + wigdata.user_time + wigdata.api_time)
                    ) {
                        wigdata.code = "10001";
                        wigdata.msg = "签名校验失败";
                        reject(wigdata);
        ...
        var app = new Vue({
            ...
            data() {
                return {
                    ...
                    bsok: false,
                    // 到期时间
                    expires: "0000年0月0日\x2000:00:00",
                };
            },
            mounted() {
                this.getCard();
                this.setRect(550, 300);
                //竖屏
